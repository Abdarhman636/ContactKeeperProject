...
export const Login = props => {
     ...
     useEffect(() => {
          ...
          if (error === 'there is something wrong with the email') {
               setAlert(error, 'danger')
               clearErrors()
          } else if ( error === 'there is something wrong with the password') {
               setAlert(error, 'danger')
               clearErrors()  
          }
     ...
     return (
          <div className='form-container'>
               <h1>Account <span className='text-primary'></span>Login</h1>
